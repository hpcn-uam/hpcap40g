...
extern struct i40e_vsi * adapters[HPCAP_MAX_NIC];
#endif /* DEV_HPCAP */
...
#endif
{
	...
	vsi->num_queue_pairs = hpcap_get_consumer_count();
#else
...
int i40e_vsi_setup_rx_resources(struct i40e_vsi *vsi)
{
	...
	for (i = 0; i < vsi->num_queue_pairs && !err; i++) {
		printk(KERN_WARNING "Setting up queue %d\n", i);
...
#ifdef DEV_HPCAP
	vsi->rx_ring = vsi->rx_rings; // Naming compatibility
#endif
...
static irqreturn_t i40e_msix_clean_rings(int irq, void *data)
{
	...
	if (is_hpcap_adapter(q_vector->vsi))
		return IRQ_HANDLED;
...
static irqreturn_t i40e_intr(int irq, void *data)
{
	...
	if (icr0 & I40E_PFINT_ICR0_QUEUE_0_MASK) {
		...
		if (!test_bit(__I40E_DOWN, &pf->state)) {
			...
			if (!is_hpcap_adapter(pf->vsi[pf->lan_vsi]))
#endif
...
static int i40e_vsi_alloc_q_vectors(struct i40e_vsi * vsi)
{
	...
	dev_info(&pf->pdev->dev, "Allocating %d q_vectors\n", num_q_vectors);
...
#if defined(HAVE_IOMMU_PRESENT) && !defined(DEV_HPCAP)
...
#ifdef DEV_HPCAP
#ifdef HPCAP_CONSUMERS_VIA_RINGS
	pf->rss_size_max = hpcap_get_consumer_count();
#else
	pf->rss_size_max = 1;
#endif
#endif
								...
								u16 uplink_seid, u32 param1)
{
	...
	vsi->bd_number = hpcap_get_iface_number_or_default(pf->pdev);

	hpcap_parse_opts(vsi);

	adapters[adapters_found] = vsi;
	adapters_found++;

	vsi->hpcap_client_discard = 0;
	vsi->hpcap_client_loss = 0;

#endif /*DEV_HPCAP*/

#ifdef DEV_HPCAP
#ifdef HPCAP_CONSUMERS_VIA_RINGS
	vsi->num_q_vectors = vsi->consumers;
#else
	vsi->num_q_vectors = 1;
#endif
#endif
	...
	if (!(pf->flags & I40E_FLAG_NO_PCI_LINK_CHECK)) {
		...
		switch (hw->bus.speed) {
			case i40e_bus_speed_8000:
				strncpy(speed, "8.0", PCI_SPEED_SIZE);
				break;

			case i40e_bus_speed_5000:
				strncpy(speed, "5.0", PCI_SPEED_SIZE);
				break;

			case i40e_bus_speed_2500:
				strncpy(speed, "2.5", PCI_SPEED_SIZE);
				break;
		...
		switch (hw->bus.width) {
			case i40e_bus_width_pcie_x8:
				strncpy(width, "8", PCI_WIDTH_SIZE);
				break;

			case i40e_bus_width_pcie_x4:
				strncpy(width, "4", PCI_WIDTH_SIZE);
				break;

			case i40e_bus_width_pcie_x2:
				strncpy(width, "2", PCI_WIDTH_SIZE);
				break;

			case i40e_bus_width_pcie_x1:
				strncpy(width, "1", PCI_WIDTH_SIZE);
				break;
...
static int __init i40e_init_module(void)
{
	...
	devs_found = hpcap_fix_iface_numbers(i40e_pci_tbl);
