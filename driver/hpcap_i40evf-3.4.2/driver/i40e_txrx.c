...
/* For HPCAP we don't want to split packets in separate memory regions. */
...
void i40e_free_rx_resources(struct i40e_ring *rx_ring)
{
	...
	for (i = 0; i < I40E_MAX_SUBWINDOWS; i++) {
		if (rx_ring->window[i]) {
...
int i40e_setup_rx_descriptors(struct i40e_ring *rx_ring)
{
	...
	dev_info(dev, "Allocated descriptors for ring %p in node %d, count %d\n", rx_ring, dev_to_node(dev), rx_ring->count);
	p = virt_to_page(rx_ring->desc);

	if (p == NULL)
		dev_info(dev, "p == NULL! desc (%p)", rx_ring->desc);
	else
		dev_info(dev, "Desc at %p has node %d\n", rx_ring->desc, page_to_nid(p));

#ifdef DEV_HPCAP
	rx_ring->queued = 0;
	rx_ring->window_size = ALIGN(I40E_SUBWINDOW_SIZE * MAX_DESCR_SIZE, 4096);

	for (i = 0; i <= (rx_ring->count - 1) / I40E_SUBWINDOW_SIZE; i++) {
		rx_ring->window[i] = dma_alloc_coherent(dev, rx_ring->window_size, &rx_ring->dma_window[i], GFP_KERNEL);//different from version 2.0.38
		p = virt_to_page(rx_ring->window[i]);

		if (p == NULL)
			dev_info(dev, "p == NULL! at window %zu (%p)", i, rx_ring->window[i]);
		else
			dev_info(dev, "Window %zu at %p has node %d\n", i, rx_ring->window[i], page_to_nid(p));
...
#ifdef DEV_HPCAP
void i40e_alloc_rx_buffers_hpcap(struct i40e_ring *rx_ring, u16 cleaned_count)
{
	...
	while (cleaned_count--) {
		...
		rx_ring->rx_bi[i].dma = packet_dma(rx_ring, i);
