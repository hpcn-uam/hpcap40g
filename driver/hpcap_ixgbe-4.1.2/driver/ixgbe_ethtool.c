...
#ifdef DEV_HPCAP
	IXGBE_STAT("rx_hpcap_client_lost_frames", hpcap_client_loss),
	IXGBE_STAT("rx_hpcap_noclient_frames", hpcap_client_discard),
#ifdef REMOVE_DUPS
	IXGBE_STAT("rx_hpcap_dup_frames", total_dup_frames)
#endif
#endif
};
							  ...
							  struct ethtool_cmd *ecmd)
{
	...
	adapter_dbg(DBG_DRV, "ethtool configuration request: speed %d duplex %d autoneg %d\n",
				(int) ecmd->speed, (int) ecmd->duplex, (int) ecmd->autoneg);

	if ((hw->phy.media_type == ixgbe_media_type_copper) ||
		(hw->phy.multispeed_fiber)) {
		adapter_dbg(DBG_DRV, "Multispeed configuration enabled: Adapter is copper/multispeed_fiber\n");
		...
		if (ecmd->advertising & ~ecmd->supported) {
			adapter_dbg(DBG_DRV, "For some reason, we are advertising features that are not supported.\n");
			return -EINVAL;
		}
		...
		if (!ecmd->autoneg && hw->phy.multispeed_fiber) {
			if (ecmd->advertising ==
				...
				DPRINTK(PROBE, WARNING, "Multispeed links not supported with autonegotiation disabled.\n");
				return -EINVAL;
			}
		}
		...
		if (old == advertised) {
			adapter_dbg(DBG_DRV, "Link speed unchanged\n");
			return 0;
		}

		adapter_dbg(DBG_DRV, "Setting link speed, restarting autonegotiation\n");
		...
		if ((ecmd->autoneg == AUTONEG_ENABLE) ||
			...
			DPRINTK(PROBE, WARNING, "Link only supports 10Gb/FULL with no autonegotiation\n");
			return -EINVAL;
		}
	}
							   ...
							   struct ethtool_channels *ch)
{
	...
	/* record RSS queues */
#ifdef DEV_HPCAP
	ch->combined_count = adapter->ring_feature[RING_F_RXQ].indices;
#else
...
#ifdef DEV_HPCAP
	adapter->ring_feature[RING_F_RXQ].limit = count;
#else
