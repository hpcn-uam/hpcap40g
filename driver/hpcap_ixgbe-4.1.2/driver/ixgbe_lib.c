...
#ifdef DEV_HPCAP
	rss_i = adapter->ring_feature[RING_F_RXQ].indices;
#else
...
static bool ixgbe_cache_ring_vmdq(struct ixgbe_adapter *adapter)
{
	...
	struct ixgbe_ring_feature *vmdq = &adapter->ring_feature[RING_F_VMDQ];
#ifdef DEV_HPCAP
	struct ixgbe_ring_feature *rss = &adapter->ring_feature[RING_F_RXQ];
#else
...
static bool ixgbe_set_dcb_vmdq_queues(struct ixgbe_adapter *adapter)
{
	 ...
	 */
#ifdef DEV_HPCAP
	adapter->ring_feature[RING_F_RXQ].indices = 1;
	adapter->ring_feature[RING_F_RXQ].mask = IXGBE_RSS_DISABLED_MASK;
#else
...
#ifdef DEV_HPCAP
	adapter->ring_feature[RING_F_RXQ].indices = rss_i;
	adapter->ring_feature[RING_F_RXQ].mask = rss_m;
#else
 ...
 * @param adapter  board private structure to initialize
 * @param v_count  q_vectors allocated on adapter, used for ring interleaving
 * @param v_idx  index of vector in adapter struct
 * @param txr_count  total number of Tx rings to allocate
 * @param txr_idx  index of first Tx ring to allocate
 * @param rxr_count  total number of Rx rings to allocate
 * @param rxr_idx  index of first Rx ring to allocate
 *
 ...
 * @param adapter  board private structure to initialize
 * @param v_idx  Index of vector to be freed
 *
...
#ifdef DEV_HPCAP
	adapter->ring_feature[RING_F_RXQ].limit = 1;
#else
