...
#define IXGBE_BUILD_INFO __stringify(4.1.2) DRIVERIOV DRV_HW_PERF FPGA \
			BYPASS_TAG RELEASE_TAG

#ifdef DEV_HPCAP

#ifndef HPCAP_BUILD_INFO
#define HPCAP_BUILD_INFO "no-hpcap-info"
#endif

#define DRV_VERSION "HPCAP " HPCAP_BUILD_INFO " IXGBE " IXGBE_BUILD_INFO
#else
#define DRV_VERSION IXGBE_BUILD_INFO
#endif
 ...
 * @param adapter  pointer to adapter struct
 * @param direction  0 for Rx, 1 for Tx, -1 for other causes
 ...
 * @param adapter  driver private struct
 **/
...
static void ixgbe_tx_timeout(struct net_device *netdev)
{
	...
	if (is_hpcap_adapter(adapter))
		return; // HPCAP adapters don't transmit anything, so don't respond to timeouts.
...
#ifdef DEV_HPCAP
bool ixgbe_clean_tx_irq_hpcap(struct ixgbe_q_vector *q_vector, struct ixgbe_ring *tx_ring)
{
 ...
 * @param q_vector  structure containing interrupt and ring information
 * @param tx_ring  tx ring to clean
 **/
bool ixgbe_clean_tx_irq_standard(struct ixgbe_q_vector *q_vector,
 ...
 * @param ring  structure containing ring specific data
 * @param rx_desc  advanced rx descriptor
 *
 ...
 * @param ring  structure containing ring specific data
 * @param rx_desc  current Rx descriptor being processed
 * @param skb  skb currently being received and modified
 **/
...
#ifdef DEV_HPCAP
void ixgbe_release_rx_desc(struct ixgbe_ring *rx_ring, u32 val)
#else
...
#ifdef DEV_HPCAP
void ixgbe_alloc_rx_buffers_hpcap(struct ixgbe_ring *rx_ring, u16 cleaned_count) //different from version 2.0.38
{
	union ixgbe_adv_rx_desc *rx_desc;
 ...
 * @param rx_ring  ring to place buffers on
 * @param cleaned_count  number of buffers to replace
 **/
 ...
 * @param tail  pointer to active tail in frag_list
 *
 ...
 * @param head  pointer to the start of the skb
 * @param tail  pointer to active tail to add to frag_list
 *
 ...
 * @param head  pointer to head of an active frag list
 *
 ...
 * @param rx_ring  structure containing ring specific data
 * @param rx_desc  pointer to the rx descriptor
 * @param skb  pointer to the skb to be merged
 *
 **/
 ...
 * @param q_vector  structure containing interrupt and ring information
 * @param new_skb  pointer to current skb being checked
 *
 ...
 * @param rx_ring  rx descriptor ring packet is being transacted on
 * @param rx_desc  pointer to the EOP Rx descriptor
 * @param skb  pointer to current skb being populated
 *
 ...
 * @param rx_ring  Rx ring being processed
 * @param rx_desc  Rx descriptor for current buffer
 * @param skb  Current socket buffer containing buffer in progress
 *
 ...
 * @param skb  pointer to current skb being adjusted
 *
 ...
 * @param rx_ring  rx descriptor ring packet is being transacted on
 * @param skb  pointer to current skb being updated
 *
 ...
 * @param rx_ring  rx descriptor ring packet is being transacted on
 * @param rx_desc  pointer to the EOP Rx descriptor
 * @param skb  pointer to current skb being fixed
 *
 ...
 * @param rx_ring  rx descriptor ring to store buffers on
 * @param old_buff  donor buffer to have page reused
 *
 ...
 * @param rx_ring  rx descriptor ring to transact packets on
 * @param rx_buffer  buffer containing page to add
 * @param rx_desc  descriptor containing length of buffer written by hardware
 * @param skb  sk_buff to place the data into
 *
 ...
 * @param q_vector  structure containing interrupt and ring information
 * @param rx_ring  rx descriptor ring to transact packets on
 * @param budget  Total limit on number of packets to process
 *
 ...
 * @param q_vector  structure containing interrupt and ring information
 * @param ring_container  structure containing ring performance data
 *
 ...
 * @param napi  napi struct with our devices info in it
 * @param budget  amount of work driver is allowed to do this pass, in packets
 *
...
static void ixgbe_setup_mtqc(struct ixgbe_adapter *adapter)
{
	...
	u32 rttdcs, mtqc;
#ifdef DEV_HPCAP
	int indices = adapter->ring_feature[RING_F_RXQ].indices;
#else
	int indices = adapter->ring_feature[RING_F_RSS].indices;
#endif
								   ...
								   struct ixgbe_ring *rx_ring)
{
	...
	if (hw->mac.type == ixgbe_mac_82598EB) {
#ifdef DEV_HPCAP
		u16 mask = adapter->ring_feature[RING_F_RXQ].mask;
#else
							 ...
							 const u32 *seed)
{
	...
	int indices_multi;
#ifdef DEV_HPCAP
	u16 rss_i = adapter->ring_feature[RING_F_RXQ].indices;
#else
...
static void ixgbe_setup_mrqc(struct ixgbe_adapter *adapter)
{
	...
	u32 vfmrqc = 0;
#ifdef DEV_HPCAP
	struct ixgbe_ring_feature* rss_feat = &adapter->ring_feature[RING_F_RXQ];
#else
	struct ixgbe_ring_feature* rss_feat = &adapter->ring_feature[RING_F_RSS];
#endif
 ...
 * @param adapter  - the private structure
 *
 ...
 * @param param  napi napi struct with our devices info in it
 * @param param  budget amount of work driver is allowed to do this pass, in packets
 *
...
static void ixgbe_napi_enable_all(struct ixgbe_adapter *adapter)
{
	...
	for (q_idx = 0; q_idx < adapter->num_q_vectors; q_idx++) {
		...
		if (is_hpcap_adapter(adapter))
			q_vector->napi.poll = &ixgbe_clean_rxonly;
 ...
 * @param adapter  ixgbe adapter struct
 *
...
static void ixgbe_configure_dcb(struct ixgbe_adapter *adapter)
{
	...
	if (hw->mac.type != ixgbe_mac_82598EB) {
		u32 msb = 0;
#ifdef DEV_HPCAP
		u16 rss_i = adapter->ring_feature[RING_F_RXQ].indices - 1;
#else
 ...
 *
 * @param adapter  board private structure to calculate for
 * @param pb  - packet buffer to calculate
 */
 ...
 * @param adapter  pointer to private adapter struct
 **/
 ...
 * @param hw  pointer to private hardware struct
 *
 ...
 * @param rx_ring  ring to free buffers from
 **/
 ...
 * @param tx_ring  ring to be cleaned
 **/
 ...
 * @param tx_ring     tx descriptor ring (for a specific queue) to setup
 *
 ...
 * @param rx_ring     rx descriptor ring (for a specific queue) to setup
 *
 * Returns 0 on success, negative on failure
 **/
 ...
 * @param tx_ring  Tx descriptor ring for a specific queue
 *
 ...
 * @param rx_ring  ring to clean the resources from
 *
...
#ifdef DEV_HPCAP
	DPRINTK(PROBE, INFO, "Interface up in mode %d.\n", adapter->work_mode);
#endif /* DEV_HPCAP */
 ...
 * @param adapter  the private adapter struct
 *
 ...
 * @param netdev  network interface device structure
 * @param stats  storage space for 64bit statistics
 *
...
void ixgbe_update_stats(struct ixgbe_adapter *adapter)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (mpc > 0)
			adapter_dbg(DBG_RX, "Warning: NIC reports losses. MPC[%d] = %d.\n", i, mpc);
 ...
 * @param adapter  - pointer to the device adapter structure
 **/
 ...
 * @param adapter  - pointer to the device adapter structure
 * @param link_speed  - pointer to a u32 to store the link_speed
 **/
...
static void ixgbe_watchdog_flush_tx(struct ixgbe_adapter *adapter)
{
	if (!netif_carrier_ok(adapter->netdev)) {
		if (ixgbe_ring_tx_pending(adapter) ||
			...
			if (is_hpcap_adapter(adapter)) {
				adapter_dbg(DBG_DRV, "TX work pending on HPCAP adapter, ignoring.\n");
				return;
			}
 ...
 * @param adapter  - the ixgbe adapter structure
 **/
 ...
 * @param work  pointer to work_struct containing our data
 **/
									...
									struct net_device *netdev)
{
	...
	if (is_hpcap_adapter(adapter)) {
		dev_kfree_skb_any(skb);
		adapter_dbg(DBG_TX, "Trying to send frames through HPCAP interface. Rejecting.\n");
		return NETDEV_TX_OK;
	}
 ...
 * @param adapter  pointer to ixgbe_adapter
 * @param tc  number of traffic classes currently enabled
 *
 ...
 * classes.
 *
 * @param netdev  net device to configure
 * @param tc  number of traffic classes to enable
 */
							  ...
							  netdev_features_t features)
{
	...
	switch (features & NETIF_F_NTUPLE) {
		...
		default:
			...
			if (adapter->ring_feature[RING_F_RXQ].limit <= 1)
#else
 ...
 * @param dev  The port's netdev
 * @param sa_family  Socket Family that VXLAN is notifiying us about
 * @param port  New UDP port number that VXLAN started listening to
 */
 ...
 * @param dev  The port's netdev
 * @param sa_family  Socket Family that VXLAN is notifying us about
 * @param port  UDP port number that VXLAN stopped listening to
 */
 ...
 * @param adapter  the adapter private structure
 * @param device_id  the device ID
 * @param subdev_id  the subsystem device ID
 *
 ...
 * @param ent  entry in ixgbe_pci_tbl
 *
 * Returns 0 on success, negative on failure
 *
								 ...
								 const struct pci_device_id __always_unused *ent)
{
	...
	adapter->numa_node = adapter->node;
...
no_info_string:
	...
	adapter_dbg(DBG_DRV, "Probe for adapter %s finished successfully.\n", pci_name(pdev));
...
static int __init ixgbe_init_module(void)
{
	...
	printk(KERN_INFO "hpcap: %s - version %s \n", ixgbe_driver_string, ixgbe_driver_version);
	printk(KERN_INFO "%s\n", ixgbe_copyright);
#else /* DEV_HPCAP */
	pr_info("%s - version %s\n", ixgbe_driver_string, ixgbe_driver_version);
	pr_info("%s\n", ixgbe_copyright);
	...
	devs_found = hpcap_fix_iface_numbers(ixgbe_pci_tbl);
